...
def visualizeHanning(window, size, frameNum):
    out_np = window.view(-1, size).unsqueeze(2).repeat((1, 1, 3)).numpy()
    out_np = (out_np * 255).astype(np.uint8)
    out_np[out_np < 0] = 0
    hanning = cv2.applyColorMap(out_np, cv2.COLORMAP_JET)
    #cv2.imwrite(f"hanning_{frameNum}.jpg", hanning)
    return hanning
