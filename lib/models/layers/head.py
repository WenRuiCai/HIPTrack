...
class CenterPredictor(nn.Module, ):
    ...
    def forward(self, x, gt_score_map=None, return_topk_boxes=False):
        """ Forward pass with input x. """
        #import pdb
        #pdb.set_trace()
        score_map_ctr, size_map, offset_map = self.get_score_map(x)
        # score_map_ctr [B 1 H W]; size_map and offet_map are all [B 2 H W]
        ...
        if not return_topk_boxes:
            topkBbox = None
        return score_map_ctr, bbox, size_map, offset_map, topkBbox
